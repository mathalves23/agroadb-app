...
from app.api.v1.deps import get_current_superuser, get_current_user, get_db
from app.core.audit import AuditAction, audit_logger
from app.core.lgpd import ConsentType, lgpd_service
...
async def confirm_two_factor_auth(
    ...
    """
    success = await two_factor_service.confirm_and_enable_2fa(db, current_user.id, data.token)
    ...
    if not success:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Código inválido. Tente novamente."
        )
    ...
    return {"status": "enabled", "message": "Autenticação de dois fatores ativada com sucesso!"}
...
async def disable_two_factor_auth(
    ...
    """
    success = await two_factor_service.disable_2fa(db, current_user.id, data.token)
    ...
    return {"status": "disabled", "message": "Autenticação de dois fatores desabilitada"}


@router.get("/2fa/status")
async def get_2fa_status(current_user: User = Depends(get_current_user), db=Depends(get_db)):
...
async def record_user_consent(
    ...
    **Tipos de consentimento**:
    ...
    if data.accept:
        ...
        return {"status": "revoked", "message": "Consentimento revogado"}
...
async def request_data_deletion(
    ...
    **Importante**:
    ...
    """
    deletion_request = await lgpd_service.request_data_deletion(db, current_user.id, data.reason)
...
async def get_deletion_requests(
    ...
    **Requer**: superutilizador (`is_superuser`).
    ...
    return {"message": "Endpoint em desenvolvimento", "requests": []}
