...
class CollaborationService:
    ...
    async def share_investigation(
        ...
        if existing_share:
            ...
            logger.info(
                f"✏️ Compartilhamento atualizado: Investigation {investigation_id} com {shared_with_email}"
            )
        ...
        logger.info(
            f"✅ Investigação {investigation_id} compartilhada com {shared_with_email} ({permission.value})"
        )

        return share

    @staticmethod
    async def revoke_access(db, investigation_id: int, owner_id: int, shared_with_id: int) -> bool:
        """
    ...
    async def check_permission(
        db, investigation_id: int, user_id: int, required_permission: PermissionLevel
    ) -> bool:
        """
    ...
    async def get_shared_investigations(
        db, user_id: int, include_owned: bool = False
    ) -> List[dict]:
        """
        ...
        query = (
            select(Investigation, InvestigationShare)
            .join(InvestigationShare, Investigation.id == InvestigationShare.investigation_id)
            .where(
                InvestigationShare.shared_with_id == user_id,
                InvestigationShare.is_active == True,
                or_(
