...
class RiskScoringEngine:
    ...
    @classmethod
    async def calculate_risk_score(cls, db, investigation_id: int) -> RiskScore:
        """
        ...
        contract_score, contract_patterns = await cls._analyze_contract_values(db, investigation)
        indicators.append(
            RiskIndicator(
                name="contract_value",
        ...
        legal_score, legal_patterns = await cls._analyze_legal_issues(db, investigation)
        indicators.append(
            RiskIndicator(
                name="legal_issues",
                value=legal_score,
        ...
        # 4. Rede de Empresas
        company_score, company_patterns = await cls._analyze_company_network(db, investigation)
        indicators.append(
            RiskIndicator(
        ...
        # 5. Padrões Temporais
        temporal_score, temporal_patterns = await cls._analyze_temporal_patterns(db, investigation)
        indicators.append(
            RiskIndicator(
        ...
        geo_score, geo_patterns = await cls._analyze_geographic_dispersion(db, investigation)
        indicators.append(
            RiskIndicator(
        ...
        # Governança / transparência (versão do motor, pesos, política organizacional)
        governance = await build_risk_governance_context(
            db,
            owner_user_id=investigation.user_id,
            indicator_weights=dict(cls.WEIGHTS),
            calibration_config=cal_cfg,
            calibration_meta=dict(cal_meta),
        )

        # Gerar recomendações
        recommendations = cls._generate_recommendations(indicators, patterns, risk_level)
        if governance.get("human_review_required"):
            recommendations = list(recommendations)
            recommendations.insert(
                0,
                "[Governança] A política da organização exige validação humana deste score "
                "automatizado (transparência / LGPD) antes de decisões de crédito, diligência "
                "final ou efeitos jurídicos.",
            )

        logger.info(
            f"✅ Risk score calculado: raw={total_raw:.2f} calibrado={total_score:.2f} "
            f"({risk_level}) para investigação {investigation_id} "
            f"motor={governance.get('engine_version')}"
        )
    ...
    async def _analyze_property_concentration(cls, db, investigation) -> Tuple[float, List[str]]:
        ...
        # Buscar propriedades
        query = select(Property).where(Property.investigation_id == investigation.id)
        result = await db.execute(query)
    ...
    async def _analyze_contract_values(cls, db, investigation) -> Tuple[float, List[str]]:
        ...
        query = select(LeaseContract).where(LeaseContract.investigation_id == investigation.id)
        result = await db.execute(query)
        ...
        if contracts:
            ...
            if values:
                ...
                if outliers:
                    score += 20
                    patterns.append(f"Detectados {len(outliers)} contratos com valores atípicos")
    ...
    async def _analyze_legal_issues(cls, db, investigation) -> Tuple[float, List[str]]:
        ...
        query = select(LegalQuery).where(LegalQuery.investigation_id == investigation.id)
        result = await db.execute(query)
        ...
        if critical_subjects >= 5:
            score += 50
            patterns.append(f"Detectados {critical_subjects} processos com assuntos críticos")
    ...
    async def _analyze_company_network(cls, db, investigation) -> Tuple[float, List[str]]:
        ...
        query = select(Company).where(Company.investigation_id == investigation.id)
        result = await db.execute(query)
        ...
        inactive_companies = sum(1 for c in companies if c.status and "inativa" in c.status.lower())
        ...
        if inactive_companies > 0:
            ...
            if inactive_ratio >= 0.5:
                score += 40
                patterns.append(
                    f"Alta taxa de empresas inativas: " f"{inactive_companies}/{num_companies}"
                )
    ...
    async def _analyze_temporal_patterns(cls, db, investigation) -> Tuple[float, List[str]]:
        ...
        query = (
            select(Company)
            .where(Company.investigation_id == investigation.id, Company.opening_date.isnot(None))
        ...
        if len(recent_properties) >= 10:
            score += 30
            patterns.append(
                f"{len(recent_properties)} propriedades registradas " "nos últimos 6 meses"
            )
    ...
    def _generate_recommendations(
        indicators: List[RiskIndicator], patterns: List[str], risk_level: str
    ) -> List[str]:
        """Gera recomendações baseadas no score"""
        recommendations = []

        if risk_level in ["critical", "high"]:
            recommendations.append("⚠️ Investigação prioritária: risco elevado detectado")
            recommendations.append("Considere análise detalhada manual dos dados")

        # Recomendações específicas por indicador
        for ind in indicators:
            if ind.severity in ["critical", "high"]:
                if ind.name == "property_concentration":
                    recommendations.append("🏞️ Investigar origem e legitimidade das propriedades")
                elif ind.name == "legal_issues":
                    recommendations.append("⚖️ Analisar processos judiciais em andamento")
                elif ind.name == "company_network":
                    recommendations.append("🏢 Mapear rede de empresas e sócios")
                elif ind.name == "contract_value":
                    recommendations.append("💰 Verificar origem dos recursos dos contratos")

        if not recommendations:
            recommendations.append("✅ Investigação dentro dos parâmetros normais")
