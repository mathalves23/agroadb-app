...
class OCREngine:
    ...
    @classmethod
    async def extract_text_from_pdf(cls, file_path: str, use_ocr: bool = True) -> OCRResult:
        """Extrai texto de PDF"""
        import time
    ...
    @classmethod
    async def extract_text_from_image(cls, file_path: str) -> OCRResult:
        ...
        try:
            ...
            logger.info(
                f"✅ OCR de imagem concluído: {len(text)} caracteres, " f"{processing_time:.2f}s"
            )
    ...
    def _extract_entities(cls, text: str) -> Dict[str, List[str]]:
        ...
        for entity_type, pattern in cls.PATTERNS.items():
            ...
            if matches:
                # Limpar e deduplicar
                cleaned_matches = list(set([cls._clean_entity(match) for match in matches]))
    ...
    async def process_document_batch(
        cls, file_paths: List[str], use_ocr: bool = True
    ) -> List[OCRResult]:
    ...
    async def search_text_in_document(
        cls, file_path: str, search_terms: List[str]
    ) -> Dict[str, List[int]]:
        ...
        for term in search_terms:
            ...
            if count > 0:
                ...
                findings[term] = {"count": count, "positions": positions[:10]}  # Primeiras 10
    ...
    async def extract_and_validate_documents(
        cls, file_path: str, expected_entities: Dict[str, str]
    ) -> Dict[str, bool]:
        """
        ...
        for entity_type, expected_value in expected_entities.items():
            ...
            found_normalized = [re.sub(r"[^\w]", "", found) for found in found_entities]
