...
class SerasaIntegration:
    ...
    async def _authenticate(self):
        ...
        try:
            ...
            async with self.session.post(self.AUTH_URL, data=data, timeout=30) as response:
    ...
    async def get_full_report(self, cpf_cnpj: str) -> Optional[SerasaReport]:
        ...
        try:
            ...
            payload = {"documento": doc_clean, "tipo": "CPF" if len(doc_clean) == 11 else "CNPJ"}
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    score = SerasaScore(
                        ...
                        probabilidade_inadimplencia=data.get("probabilidadeInadimplencia", 0.0),
    ...
    def _parse_serasa_report(self, data: Dict) -> SerasaReport:
        ...
        if data.get("score"):
            score = SerasaScore(
                ...
                probabilidade_inadimplencia=data["score"].get("probabilidadeInadimplencia", 0.0),
                data_consulta=datetime.now(),
            )
...
class BoaVistaIntegration:
    ...
    async def get_full_report(self, cpf_cnpj: str) -> Optional[BoaVistaReport]:
        ...
        try:
            ...
            payload = {"documento": doc_clean, "produto": "RELATORIO_COMPLETO"}
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    logger.info(
                        f"✅ Boa Vista: Relatório obtido - " f"Score: {report.score or 'N/A'}"
                    )
...
class UnifiedCreditSearch:
    ...
    @classmethod
    async def search_all_bureaus(cls, cpf_cnpj: str) -> Dict:
        """Busca em Serasa e Boa Vista simultaneamente"""

        results = {"serasa": None, "boa_vista": None, "summary": {}}
        ...
        results["summary"] = cls._generate_summary(results["serasa"], results["boa_vista"])
    ...
    def _generate_summary(
        serasa: Optional[SerasaReport], boavista: Optional[BoaVistaReport]
    ) -> Dict:
        ...
        if boavista and boavista.score:
            ...
            summary["tem_acoes_judiciais"] = (
                summary["tem_acoes_judiciais"] or len(boavista.acoes_judiciais) > 0
            )
            summary["tem_cheques_sem_fundo"] = (
                summary["tem_cheques_sem_fundo"] or len(boavista.cheques_sem_fundo) > 0
            )
