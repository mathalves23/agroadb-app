...
class IBAMAIntegration:
    ...
    async def search_embargos(self, cpf_cnpj: str) -> List[IBAMAEmbargo]:
        ...
        try:
            ...
            params = {"tipoConsulta": "documento", "documento": doc_clean}
            ...
            async with self.session.get(
                self.EMBARGOS_URL, params=params, headers=headers, timeout=30
            ) as response:
                ...
                if response.status == 200:
                    ...
                    logger.info(f"✅ IBAMA: {len(embargos)} embargos encontrados")
...
class FUNAIIntegration:
    ...
    async def search_indigenous_lands(
        self, municipio: Optional[str] = None, uf: Optional[str] = None
    ) -> List[FUNAILand]:
        ...
        try:
            ...
            async with self.session.get(self.GEOSERVER_URL, params=params, timeout=30) as response:
                ...
                if response.status == 200:
                    ...
                    logger.info(f"✅ FUNAI: {len(lands)} terras indígenas encontradas")
    ...
    async def check_overlap_with_property(self, car_number: str) -> Dict:
        ...
        try:
            ...
            async with self.session.get(self.GEOSERVER_URL, params=params, timeout=30) as response:
                ...
                if response.status == 200:
                    ...
                    logger.info(f"✅ ICMBio: {len(units)} unidades de conservação encontradas")
...
class SPUIntegration:
    ...
    async def search_federal_properties(self, municipio: str, uf: str) -> List[SPUProperty]:
    ...
    async def check_federal_land(self, car_number: str) -> Dict:
