...
class SerasaService:
    ...
    async def _authenticate(self):
        ...
        if not self.client_id or not self.client_secret:
            ...
            logger.info("Configure SERASA_CLIENT_ID e SERASA_CLIENT_SECRET no .env")
            return
    ...
    async def consultar_score(self, cpf_cnpj: str) -> Optional[SerasaScore]:
        """
    ...
    async def consultar_restricoes(self, cpf_cnpj: str) -> List[SerasaRestriction]:
        """
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    logger.info(f"✅ Serasa: {len(restricoes)} restrições encontradas")
    ...
    async def consultar_consultas_recentes(self, cpf_cnpj: str) -> List[SerasaConsulta]:
        """
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    logger.info(f"✅ Serasa: {len(consultas)} consultas recentes")
