...
class PJeIntegration:
    ...
    async def search_processes_by_cpf(self, cpf: str) -> List[PJeProcess]:
        ...
        try:
            ...
            async with self.session.get(
                ...
                if response.status == 200:
                    ...
                    logger.info(f"✅ PJe: {len(processes)} processos encontrados para CPF {cpf}")
                    ...
                    logger.error(f"Erro PJe {response.status}: {error_text}")
                    return []
    ...
    async def search_processes_by_cnpj(self, cnpj: str) -> List[PJeProcess]:
        ...
        try:
            ...
            async with self.session.get(
                ...
                if response.status == 200:
                    ...
                    logger.info(f"✅ PJe: {len(processes)} processos encontrados para CNPJ {cnpj}")
    ...
    async def get_process_details(self, numero_processo: str) -> Optional[PJeProcess]:
    ...
    def _parse_process(self, data: Dict) -> PJeProcess:
        ...
        if data.get("dataAutuacao"):
            try:
                data_autuacao = datetime.fromisoformat(data["dataAutuacao"].replace("Z", "+00:00"))
            except:
                pass
    ...
    async def consultar_todos_tribunais(self, cpf_cnpj: str) -> Dict[str, List[PJeProcess]]:
        """
        ...
        logger.info(
            f"✅ PJe: Busca completa - {total_processes} processos " f"em {len(results)} tribunais"
        )
