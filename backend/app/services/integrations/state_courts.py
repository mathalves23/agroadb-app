...
class ESAJIntegration:
    ...
    async def search_processes(
        self, tribunal: str, documento: str, tipo: str = "parte"  # CPF ou CNPJ
        ...
        try:
            ...
            async with self.session.get(
                ...
                if response.status == 200:
                    ...
                    logger.info(f"✅ {tribunal}: {len(processes)} processos encontrados")
...
class ProjudiIntegration:
    ...
    async def search_processes(self, tribunal: str, documento: str) -> List[StateCourtProcess]:
        ...
        try:
            ...
            data = {"tipoConsulta": "PARTE", "documento": doc_clean, "metodo": "pesquisar"}
            ...
            async with self.session.post(
                search_url, data=data, headers=headers, timeout=30
            ) as response:
                ...
                if response.status == 200:
                    ...
                    logger.info(f"✅ {tribunal} (Projudi): {len(processes)} processos encontrados")
    ...
    def _parse_projudi_html(self, html: str, tribunal: str) -> List[StateCourtProcess]:
...
class UnifiedStateCourtSearch:
    ...
    async def search_all_tribunals(
        cls, documento: str, tribunais: Optional[List[str]] = None
