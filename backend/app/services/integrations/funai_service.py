...
class FUNAIService:
    ...
    async def consultar_terras_indigenas(
        self, municipio: Optional[str] = None, uf: Optional[str] = None, nome: Optional[str] = None
    ) -> List[TerraIndigena]:
        """
    ...
    async def verificar_sobreposicao_por_coordenadas(
        self, latitude: float, longitude: float, raio_km: float = 10.0
        ...
        try:
            logger.info(
                f"Verificando sobreposição com terras indígenas " f"em ({latitude}, {longitude})"
            )
            ...
            async with self.session.get(
                self.GEOSERVER_URL, params=params, timeout=aiohttp.ClientTimeout(total=30)
            ) as response:

                if response.status != 200:
                    logger.warning(f"FUNAI retornou status {response.status}")
                    return SobreposicaoTerraIndigena(tem_sobreposicao=False, terras_sobrepostas=[])
                ...
                return SobreposicaoTerraIndigena(
                    tem_sobreposicao=tem_sobreposicao, terras_sobrepostas=terras
                )
    ...
    async def listar_etnias(self, uf: Optional[str] = None) -> List[str]:
        ...
        try:
            ...
            etnias = sorted(
                set(terra.etnia for terra in terras if terra.etnia and terra.etnia != "N/A")
            )
