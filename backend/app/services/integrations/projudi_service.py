...
class ProjudiService:
    ...
    async def consultar_processos(
        self, cpf_cnpj: str, tribunal: str = "tjmt"
    ) -> List[ProjudiProcess]:
        """
        ...
        try:
            ...
            processes = await self._search_http(search_url, doc_clean, tribunal_lower, base_url)
            ...
            logger.info(f"✅ {tribunal.upper()} (Projudi): {len(processes)} processos encontrados")
    ...
    async def _search_http(
        self, url: str, documento: str, tribunal: str, base_url: str
    ) -> List[ProjudiProcess]:
        """Busca usando requisição HTTP direta"""

        try:
            data = {"tipoConsulta": "PARTE", "documento": documento, "metodo": "pesquisar"}
                ...
                try:
                    ...
                    try:
                        radio_parte = wait.until(
                            EC.presence_of_element_located((By.XPATH, "//input[@value='PARTE']"))
                        )
                        radio_parte.click()
                    except:
                    ...
                    try:
                        wait.until(
                            EC.presence_of_element_located((By.CLASS_NAME, "resultadoConsulta"))
                        )
    ...
    def _parse_projudi_html(self, html: str, tribunal: str, base_url: str) -> List[ProjudiProcess]:
        ...
        try:
            ...
            for result in results:
                ...
                numero_matches = re.findall(r"\d{7}-\d{2}\.\d{4}\.\d\.\d{2}\.\d{4}", result_text)

                if not numero_matches:
                    # Tentar formato alternativo
                    numero_matches = re.findall(r"\d{6,8}-?\d{2}\.?\d{4}", result_text)
                ...
                if partes_text:
                    ...
                    for line in partes_lines:
                        if ":" in line:
                            ...
                            partes.append({"tipo": tipo.strip(), "nome": nome.strip()})
                ...
                if mov_section:
                    ...
                    for mov in mov_items[:10]:  # Últimas 10
                        movimentacoes.append(
                            {
                                "data": (
                                    mov.find("span", class_="data").get_text(strip=True)
                                    if mov.find("span", class_="data")
                                    else ""
                                ),
                                "descricao": (
                                    mov.find("div", class_="texto").get_text(strip=True)
                                    if mov.find("div", class_="texto")
                                    else mov.get_text(strip=True)
                                ),
                            }
                        )
