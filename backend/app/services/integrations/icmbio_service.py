...
class ICMBioService:
    ...
    async def consultar_unidades_conservacao(
        ...
        try:
            ...
            async with self.session.get(
                ...
                logger.info(
                    f"✅ ICMBio: {len(unidades)} unidade(s) de " f"conservação encontrada(s)"
                )
    ...
    async def verificar_sobreposicao_por_coordenadas(
        ...
        """
        try:
            logger.info(f"Verificando sobreposição com UCs " f"em ({latitude}, {longitude})")
            ...
            async with self.session.get(
                ...
                if response.status != 200:
                    logger.warning(f"ICMBio retornou status {response.status}")
                    return SobreposicaoUC(tem_sobreposicao=False, unidades_sobrepostas=[])
                ...
                return SobreposicaoUC(
                    tem_sobreposicao=tem_sobreposicao, unidades_sobrepostas=unidades
                )
    ...
    async def listar_categorias(self, grupo: Optional[str] = None) -> List[str]:
        ...
        try:
            ...
            categorias = sorted(
                set(uc.categoria for uc in unidades if uc.categoria and uc.categoria != "N/A")
            )
