...
class ReceitaScraper(BaseScraper):
    ...
    async def _fetch_from_provider(
        self, provider: APIProvider, cnpj: str
        ...
        try:
            if provider == APIProvider.BRASILAPI:
                ...
                response = await self.fetch(url, params={"cnpj": cnpj}, timeout=timeout)
            else:
                return None
    ...
    def _process_company_data(self, data: Dict[str, Any], provider: APIProvider) -> Dict[str, Any]:
        """
    ...
    def _extract_partners(
        self, data: Dict[str, Any], provider: APIProvider
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _extract_related_cnpjs(
        self, data: Dict[str, Any], partners: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def get_full_corporate_structure(self, cnpj: str, depth: int = 2) -> Dict[str, Any]:
        """
        ...
        for related in related_cnpjs:
            ...
            if related_cnpj:
                related_structure = await self.get_full_corporate_structure(related_cnpj, depth - 1)
    ...
    async def analyze_corporate_network(self, cnpj: str) -> Dict[str, Any]:
        ...
        common_partners = {
            doc: companies for doc, companies in all_partners.items() if len(companies) > 1
        }
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        valid_entries = sum(
            1 for timestamp, _ in self.cache.values() if datetime.now() - timestamp < self.cache_ttl
        )
