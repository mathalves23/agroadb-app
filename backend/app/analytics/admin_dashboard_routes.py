...
async def get_average_completion_time(
    ...
    group_by: Optional[str] = Query(
        None, regex="^(category|priority|user)$", description="Agrupamento opcional"
    ),
    db: Session = Depends(get_db),
    current_user: User = Depends(require_admin),
):
    ...
    **Requer:** Permissões de administrador
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro ao calcular tempo de conclusão: {str(e)}"
        )
    ...
    **Requer:** Permissões de administrador
    ...
    try:
        ...
        if format == "json":
            ...
            writer.writerow(
                ["Total de Investigações", data["platform_metrics"]["investigations"]["total"]]
            )
            writer.writerow(
                [
                    "Investigações Concluídas",
                    data["platform_metrics"]["investigations"]["completed"],
                ]
            )
            writer.writerow(
                ["Taxa de Conclusão (%)", data["conversion_rate"]["conversion_rates"]["completion"]]
            )
