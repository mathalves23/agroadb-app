...
class AdminDashboard:
    ...
    async def get_platform_metrics(
        ...
        try:
            ...
            total_investigations = _sql_count_int(
                self.db.query(func.count(Investigation.id)).scalar()
            )

            # Investigações criadas no período
            investigations_period = _sql_count_int(
            ...
            completed_investigations = _sql_count_int(
                self.db.query(func.count(Investigation.id))
                .filter(
                    and_(
            ...
            active_investigations = _sql_count_int(
                self.db.query(func.count(Investigation.id))
                .filter(
            ...
            user_growth_rate = (
                ((new_users - previous_period_users) / previous_period_users * 100)
                if previous_period_users > 0
                else 0
            )

            # Taxa de crescimento de investigações
            previous_period_investigations = (
                _sql_count_int(
                    self.db.query(func.count(Investigation.id))
                    .filter(
                        and_(
            ...
            investigation_growth_rate = (
                (
                    (investigations_period - previous_period_investigations)
                    / previous_period_investigations
                    * 100
                )
                if previous_period_investigations > 0
                else 0
            )
            ...
            return {
                ...
                "investigations": {
                    ...
                    "completion_rate": round(
                        (
                            (completed_investigations / investigations_period * 100)
                            if investigations_period > 0
                            else 0
                        ),
                        2,
                    ),
        ...
        try:
            ...
            query = (
                self.db.query(
                    ...
                    func.count(
                        case((Investigation.status == InvestigationStatus.COMPLETED, 1))
                    ).label("completed"),
                    func.count(
                        case((Investigation.status == InvestigationStatus.IN_PROGRESS, 1))
                    ).label("in_progress"),
                    func.count(
                        case((Investigation.status == InvestigationStatus.PENDING, 1))
                    ).label("pending"),
                    func.count(case((Investigation.status == InvestigationStatus.FAILED, 1))).label(
                        "cancelled"
                    ),
                )
                .filter(
                ...
                data.append(
                    {
                        ...
                        "completion_rate": round(
                            (row.completed / row.total * 100) if row.total > 0 else 0, 2
                        ),
                    }
                )
        ...
        try:
            ...
            if not investigations:
                return {
                    ...
                    "total_completed": 0,
                    "average_time": {"days": 0, "hours": 0, "minutes": 0},
                }
            ...
            result = {
                ...
                "median_time_seconds": (
                    int(sorted(completion_times)[len(completion_times) // 2])
                    if completion_times
                    else 0
                ),
        ...
        try:
            ...
            funnel = [
                {"stage": "created", "count": created, "percentage": 100.0},
                {
                    "stage": "in_progress",
                    "count": in_progress,
                    "percentage": round(in_progress_rate, 2),
                },
    ...
    async def get_most_active_users(
        ...
        try:
            ...
            user_investigations = (
                ...
                .order_by(desc("investigations_created"))
                .limit(limit)
                .all()
            )
            ...
            return {
                ...
                "top_user_percentage": round(
                    (
                        (users[0]["investigations_created"] / total_investigations * 100)
                        if users and total_investigations > 0
                        else 0
                    ),
                    2,
                ),
    ...
    async def get_most_used_scrapers(
        ...
        try:
            ...
            # Ordena e limita
            sorted_scrapers = sorted(scrapers_data, key=lambda x: x["executions"], reverse=True)[
                :limit
            ]
            ...
            return {
                ...
                "average_success_rate": round(
                    (
                        sum(s["success_rate"] for s in sorted_scrapers) / len(sorted_scrapers)
                        if sorted_scrapers
                        else 0
                    ),
                    2,
        ...
        try:
            ...
            return {
                ...
                "average_response_time_ms": round(
                    (
                        sum(s["avg_response_time_ms"] for s in sorted_sources) / len(sorted_sources)
                        if sorted_sources
                        else 0
                    ),
                    2,
                ...
                "average_success_rate": round(
                    (
                        sum(s["success_rate"] for s in sorted_sources) / len(sorted_sources)
                        if sorted_sources
                        else 0
                    ),
                    2,
    ...
    async def get_complete_dashboard(
        ...
        try:
            ...
            investigations_by_period = await self.get_investigations_by_period(
