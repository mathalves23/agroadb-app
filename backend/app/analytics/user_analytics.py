...
class FunnelAnalytics:
    ...
    def analyze_funnel(
        ...
        for i, step_name in enumerate(funnel_config["steps"]):
            step_data = self._analyze_step(
                step_name, i, start_date, end_date, previous_users, user_segment
            )
        ...
        avg_time = (
            sum(s.average_time_seconds for s in steps_data) / len(steps_data) if steps_data else 0
        )
    ...
    def _analyze_step(
        ...
        if step_name in ["landing_page", "signup"]:
            ...
            users_entered = (
                self.db.query(func.count(func.distinct(Investigation.user_id)))
                .filter(
                    and_(
                        Investigation.created_at >= start_date, Investigation.created_at <= end_date
                    )
                )
                .scalar()
            )
    ...
    def _identify_bottlenecks(self, steps: List[FunnelStep]) -> List[str]:
        ...
        for step in steps:
            ...
            if step.drop_off_rate > 30:
                bottlenecks.append(f"{step.step_name}: {step.drop_off_rate:.1f}% de abandono")
    ...
    def _generate_funnel_recommendations(
        self, steps: List[FunnelStep], bottlenecks: List[str]
    ) -> List[str]:
        ...
        overall_conversion = (
            (steps[-1].users_completed / steps[0].users_entered * 100)
            if steps[0].users_entered > 0
            else 0
        )
        ...
        return (
            recommendations
            if recommendations
            else ["✅ Funil operando dentro dos parâmetros esperados."]
        )
    ...
    def get_funnel_comparison(
        ...
        time_diff = (
            funnel2.average_completion_time_seconds - funnel1.average_completion_time_seconds
        )
        ...
        return {
            ...
            "changes": {
                ...
                "conversion_rate_change_pct": round(
                    (
                        (conversion_diff / funnel1.overall_conversion_rate * 100)
                        if funnel1.overall_conversion_rate > 0
                        else 0
                    ),
                    2,
                ),
                ...
                "trend": (
                    ...
                    else "declining" if conversion_diff < 0 else "stable"
                ),
            },
        }
...
class FeatureAdoptionAnalytics:
    ...
    def analyze_feature_adoption(
        ...
        for feature_key, feature_name in self.FEATURES.items():
            usage = self._analyze_feature_usage(
                feature_key, feature_name, start_date, end_date, total_users
            )
        ...
        # Estatísticas gerais
        avg_adoption = (
            sum(f.adoption_rate for f in features_data) / len(features_data) if features_data else 0
        )
    ...
    def _analyze_feature_usage(
        ...
        if feature_key == "investigations":
            ...
            total_investigations = (
                self.db.query(func.count(Investigation.id))
                .filter(
                    and_(
                        Investigation.created_at >= start_date, Investigation.created_at <= end_date
                    )
            ...
            power_users_query = (
                self.db.query(Investigation.user_id, func.count(Investigation.id).label("count"))
                .filter(
                    and_(
                        Investigation.created_at >= start_date, Investigation.created_at <= end_date
                    )
                )
                .group_by(Investigation.user_id)
                .having(func.count(Investigation.id) > 10)
                .all()
            )
            try:
    ...
    def _generate_adoption_recommendations(self, features: List[FeatureUsage]) -> List[str]:
        ...
        return recommendations if recommendations else ["✅ Adoção de features saudável em geral."]

    def get_user_adoption_profile(self, user_id: int) -> Dict[str, Any]:
        ...
        if inv_count > 0:
            ...
            features_used.append(
                {"feature": "Investigações", "usage_count": inv_count, "status": status}
            )
        ...
        for feat_name, count, status in other_features:
            features_used.append({"feature": feat_name, "usage_count": count, "status": status})
    ...
    def _determine_user_feature_status(self, usage_count: int, feature_key: str) -> str:
    ...
    def _classify_user_type(self, features_used: int, adoption_score: float) -> str:
    ...
    def _generate_user_recommendations(
        self, features_used: List[Dict], adoption_score: float
    ) -> List[str]:
        ...
        return (
            recommendations if recommendations else ["✅ Você está aproveitando bem a plataforma!"]
        )
