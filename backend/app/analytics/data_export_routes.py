    ...
    **Exemplo:**
    ...
    try:
        ...
        config = DataWarehouseConfig(
            warehouse_type=DataWarehouseType.BIGQUERY, project_id=project_id, dataset=dataset
        )
        ...
        logger.info(
            f"BigQuery export by {current_user.username}: {table_name} ({len(data)} records)"
        )
...
async def export_to_redshift(
    ...
    **Exemplo:**
    ...
    try:
        ...
        logger.info(
            f"Redshift export by {current_user.username}: {table_name} ({len(data)} records)"
        )
...
async def schedule_tableau_refresh(
    ...
    cron_schedule: str = Query(
        "0 8 * * *", description="Cron schedule (padrão: diariamente às 8h)"
    ),
    db: Session = Depends(get_db),
...
async def create_powerbi_dataset(
    ...
    **Exemplo:**
    ...
    try:
        ...
        config = PowerBIDataset(dataset_name=dataset_name, tables=tables)
...
async def preview_dataset(
    ...
    **Exemplo:**
    ...
    try:
        ...
        return {"dataset_name": dataset_name, "rows_returned": len(data), "preview": data}
