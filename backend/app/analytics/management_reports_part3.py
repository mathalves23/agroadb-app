...
class ErrorReport:
    ...
    def __init__(self, db: Session):
        ...
        self.ERROR_COSTS = {"low": 5.00, "medium": 25.00, "high": 100.00, "critical": 500.00}
    ...
    def _collect_errors(
        self, start_date: datetime, end_date: datetime, severity: Optional[str]
    ) -> List[ErrorMetrics]:
    ...
    def _get_most_critical_errors(self, errors: List[ErrorMetrics]) -> List[Dict]:
        ...
        sorted_critical = sorted(
            critical, key=lambda x: (x.severity == "critical", x.error_count), reverse=True
        )[:5]
        ...
        return [
            {
                ...
                "frequency_per_day": round(
                    e.error_count / ((e.last_occurrence - e.first_occurrence).days or 1), 2
                ),
                "avg_time_to_fix": round(e.average_time_to_fix_minutes, 2),
            }
        ...
        return {
            ...
            "by_severity": {
                ...
                for severity, cost in sorted(
                    cost_by_severity.items(), key=lambda x: x[1], reverse=True
                )
            },
            "cost_per_error": round(total_cost / len(errors), 2) if errors else 0,
            "preventable_cost": round(
                sum(e.estimated_cost_impact for e in errors if e.is_recurring) * 0.7, 2
            ),
        }
    ...
    def _generate_error_recommendations(self, errors: List[ErrorMetrics]) -> List[str]:
        ...
        return (
            recommendations
            if recommendations
            else ["✅ Erros dentro dos níveis aceitáveis. Continuar monitoramento."]
        )
...
class ManagementReportsConsolidator:
    ...
    def generate_executive_management_report(
        ...
        operational_health_score = self._calculate_operational_health(performance, uptime, errors)
        ...
        return {
            ...
            "reports": {"performance": performance, "uptime": uptime, "errors": errors},
    ...
    def _calculate_operational_health(self, performance: Dict, uptime: Dict, errors: Dict) -> Dict:
    ...
    def _prioritize_recommendations(self, recommendations: List[str]) -> List[Dict]:
        ...
        for rec in recommendations:
            ...
            prioritized.append({"priority": priority, "message": rec})

        # Ordenar por prioridade e retornar top 10
        return sorted(prioritized, key=lambda x: x["priority"])[:10]

    def _extract_key_metrics(self, performance: Dict, uptime: Dict, errors: Dict) -> Dict:
    ...
    def _generate_executive_summary(
        self, performance: Dict, uptime: Dict, errors: Dict, health_score: Dict
    ) -> str:
