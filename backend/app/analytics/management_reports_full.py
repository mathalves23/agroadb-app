...
class ManagementReports:
            ...
            if group_by:
                ...
                for key, data in grouped.items():
                    grouped_data[key] = {
                        ...
                        "average_cost": round(
                            data["total_cost"] / data["count"] if data["count"] > 0 else 0, 2
                        ),
                    }
                ...
                "summary": {
                    ...
                    "cost_breakdown_percentage": {
                        "infrastructure": round(
                            (
                                (cost_breakdown["infrastructure"] / cost_breakdown["total"] * 100)
                        ...
                        "scrapers": round(
                            (
                                (cost_breakdown["scrapers"] / cost_breakdown["total"] * 100)
                                if cost_breakdown["total"] > 0
                                else 0
                            ),
                            2,
                        ),
                        "human_resources": round(
                            (
                                (cost_breakdown["human_resources"] / cost_breakdown["total"] * 100)
                                if cost_breakdown["total"] > 0
                                else 0
                            ),
                            2,
                        ...
                        "integrations": round(
                            (
                                (cost_breakdown["integrations"] / cost_breakdown["total"] * 100)
                                if cost_breakdown["total"] > 0
                                else 0
                            ),
                            2,
                        ...
                        "storage": round(
                            (
                                (cost_breakdown["storage"] / cost_breakdown["total"] * 100)
                                if cost_breakdown["total"] > 0
                                else 0
                            ),
                            2,
    ...
    async def get_scraper_performance(
        ...
        try:
            ...
            scrapers_performance = [
                {
                    ...
                    "total_data_mb": 450.2,
                    "errors": {"timeout": 32, "connection_error": 15, "parse_error": 9},
                ...
                },
                {
                    ...
                    "total_data_mb": 320.5,
                    "errors": {"timeout": 8, "connection_error": 6, "parse_error": 4},
                ...
                },
                {
                    ...
                    "total_data_mb": 680.3,
                    "errors": {"timeout": 45, "connection_error": 12, "parse_error": 10},
                ...
                },
                {
                    ...
                    "total_data_mb": 540.1,
                    "errors": {"timeout": 25, "connection_error": 10, "parse_error": 6},
                ...
                },
                {
                    ...
                    "total_data_mb": 890.4,
                    "errors": {"timeout": 38, "connection_error": 18, "parse_error": 11},
            ...
            avg_success_rate = sum(s["success_rate"] for s in scrapers_performance) / len(
                scrapers_performance
            )
            avg_duration = sum(s["avg_duration_seconds"] for s in scrapers_performance) / len(
                scrapers_performance
            )
            total_data = sum(s["total_data_mb"] for s in scrapers_performance)
            total_cost = sum(
                s["total_executions"] * s["cost_per_execution"] for s in scrapers_performance
            )
            ...
            problematic = [
                s
                for s in scrapers_performance
                if s["success_rate"] < 95 or s["uptime_percentage"] < 97
            ]
            top_performers = sorted(
                scrapers_performance, key=lambda x: x["success_rate"], reverse=True
            )[:3]
    ...
    def _generate_scraper_recommendations(self, scrapers: List[Dict]) -> List[str]:
        ...
        if not recommendations:
            recommendations.append(
                "✅ Todos os scrapers estão operando dentro dos parâmetros esperados."
            )
    ...
    async def get_uptime_availability(
        ...
        try:
            ...
            sla_target = 99.5
            overall_uptime = sum(comp["uptime_percentage"] for comp in uptime_data.values()) / len(
                uptime_data
            )
            ...
            return {
                ...
                "overall": {
                    ...
                    "health_status": (
                        ...
                        else "degraded" if overall_uptime >= 95 else "critical"
                    ),
                },
    ...
    def _generate_error_recommendations(
        self, by_type: Dict[str, int], by_component: Dict[str, int]
    ) -> List[str]:
        ...
        problematic_component = (
            max(by_component.items(), key=lambda x: x[1])[0] if by_component else None
        )
            ...
            return {
                ...
                "overall_health": {
                    ...
                    "recommendations": self._generate_overall_recommendations(
                        roi, costs, scrapers, uptime, errors
                    ),
                },
            }
    ...
    def _calculate_health_score(
        self, roi: Dict, costs: Dict, scrapers: Dict, uptime: Dict, errors: Dict
    ) -> float:
    ...
    def _generate_overall_recommendations(
        ...
        if roi["summary"]["average_roi"] < 100:
            recommendations.append(
                "📊 ROI médio abaixo de 100%. Revisar processos e otimizar custos."
            )
        ...
        if total_cost and costs["summary"]["cost_breakdown"]["human_resources"] / total_cost > 0.7:
            recommendations.append(
                "💰 Custos de RH representam >70% do total. Considerar automações."
            )
        ...
        if len(scrapers["problematic_scrapers"]) > 2:
            recommendations.append(
                f"🔧 {len(scrapers['problematic_scrapers'])} scrapers problemáticos. Priorizar correções."
            )

        # Erros
        if errors["summary"]["error_rate_per_hour"] > 5:
            recommendations.append(
                "🐛 Taxa de erros elevada. Implementar melhorias em tratamento de erros."
            )
