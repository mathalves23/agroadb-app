...
class UserAnalytics:
    ...
    async def get_usage_funnel(
        ...
        try:
            ...
            users_first_investigation = _count_int(
                self.db.query(func.count(User.id.distinct()))
            ...
            active_recurring = _count_int(
                self.db.query(func.count(User.id.distinct()))
                .join(Investigation, Investigation.user_id == User.id)
                .filter(and_(User.created_at >= start_date, User.created_at <= end_date))
                .group_by(User.id)
                .having(func.count(Investigation.id) >= 5)
                .scalar()
            )
            ...
            funnel_stages = [
                ...
                },
                {
                    ...
                    "percentage": round(
                        (users_first_login / total_registered * 100) if total_registered > 0 else 0,
                        2,
                    ),
                ...
                },
                {
                    ...
                    "percentage": round(
                        (
                            (users_first_investigation / total_registered * 100)
                    ...
                    "conversion_from_previous": round(
                        (
                            (users_first_investigation / users_first_login * 100)
                            if users_first_login > 0
                            else 0
                        ),
                        2,
                    ),
                ...
                },
                {
                    ...
                    "percentage": round(
                        (
                            (users_first_collaboration / total_registered * 100)
                            if total_registered > 0
                            else 0
                        ),
                        2,
                    ...
                    "conversion_from_previous": round(
                        (
                            (users_first_collaboration / users_first_investigation * 100)
                            if users_first_investigation > 0
                            else 0
                        ),
                        2,
                    ),
                ...
                },
                {
                    ...
                    "percentage": round(
                        (
                            (users_first_report / total_registered * 100)
                            if total_registered > 0
                            else 0
                        ),
                        2,
                    ...
                    "conversion_from_previous": round(
                        (
                            (users_first_report / users_first_collaboration * 100)
                            if users_first_collaboration > 0
                            else 0
                        ),
                        2,
                    ),
                ...
                },
                {
                    ...
                    "percentage": round(
                        (active_recurring / total_registered * 100) if total_registered > 0 else 0,
                        2,
                    ),
                    "conversion_from_previous": round(
                        (
                            (active_recurring / users_first_report * 100)
                            if users_first_report > 0
                            else 0
                        ),
                        2,
                    ),
            ...
            return {
                ...
                "summary": {
                    ...
                    "average_stage_conversion": round(
                        sum(s["conversion_from_previous"] for s in funnel_stages[1:])
                        / (len(funnel_stages) - 1),
                        2,
                    ),
                },
    ...
    async def get_feature_adoption(
        ...
        try:
            ...
            total_users = (
                self.db.query(func.count(User.id))
                .filter(and_(User.created_at <= end_date, User.is_active == True))
                .scalar()
                or 0
            )
            ...
            for feature in features:
                feature["adoption_rate"] = round(
                    (feature["users_using"] / total_users * 100) if total_users > 0 else 0, 2
                )
            ...
            for cat in by_category:
                by_category[cat]["avg_adoption"] = round(
                    by_category[cat]["avg_adoption"] / by_category[cat]["features"], 2
                )
            ...
            return {
                ...
                "summary": {
                    ...
                    "average_adoption_rate": round(
                        sum(f["adoption_rate"] for f in features) / len(features), 2
                    ),
                    "features_above_80_adoption": len(
                        [f for f in features if f["adoption_rate"] > 80]
                    ),
        ...
        try:
            ...
            heatmap_data = {
                "/dashboard": {
                    ...
                    "scroll_depth": {"0-25%": 100, "25-50%": 85, "50-75%": 62, "75-100%": 38},
                },
                "/investigations": {
                    "page_views": 12890,
                    "unique_visitors": 785,
                    ...
                    "scroll_depth": {"0-25%": 100, "25-50%": 92, "50-75%": 78, "75-100%": 55},
                },
                "/reports": {
                    "page_views": 8750,
                    "unique_visitors": 625,
                    ...
                    "scroll_depth": {"0-25%": 100, "25-50%": 88, "50-75%": 72, "75-100%": 45},
                },
                "/settings": {
                    "page_views": 3250,
                    "unique_visitors": 458,
                    ...
                    "scroll_depth": {"0-25%": 100, "25-50%": 95, "50-75%": 85, "75-100%": 68},
                },
            }
            ...
            for page_name, page_data in data.items():
                for click in page_data["clicks"]:
                    all_clicks.append({"page": page_name, **click})
    ...
    async def get_nps_score(
        ...
        try:
            ...
            surveys = [
                {
                    "user_id": i,
                    "score": score,
                    "submitted_at": datetime.utcnow() - timedelta(days=i % 60),
                }
            ...
            # Calcula score médio
            avg_score = (
                sum(s["score"] for s in surveys) / total_responses if total_responses > 0 else 0
            )
            ...
            return {
                ...
                "distribution": {
                    "promoters": {"count": len(promoters), "percentage": round(promoters_pct, 2)},
                    "passives": {"count": len(passives), "percentage": round(passives_pct, 2)},
                    "detractors": {
                ...
                "recommendations": self._generate_nps_recommendations(
                    nps_score, promoters_pct, detractors_pct
                ),
            }
    ...
    def _generate_nps_recommendations(
        self, nps: float, promoters_pct: float, detractors_pct: float
    ) -> List[str]:
        ...
        if nps >= 70:
            recommendations.append("✅ NPS excelente! Considerar programa de referral/advocacy.")
            ...
            return {
                ...
                "overall_health": {
                    ...
                    "engagement_score": round(
                        (
                            nps["nps"]["score"]
                            + adoption["summary"]["average_adoption_rate"]
                            + funnel["summary"]["overall_conversion_rate"]
                        )
                        / 3,
                        2,
                    ),
