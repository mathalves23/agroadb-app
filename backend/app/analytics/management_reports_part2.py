...
from app.analytics.management_reports import ErrorMetrics, ScraperPerformanceMetrics, UptimeMetrics
...
class ScraperPerformanceReport:
    ...
    def _calculate_scraper_performance(
        self, scraper_name: str, start_date: datetime, end_date: datetime
        ...
        if durations:
            ...
            p95_duration = (
                statistics.quantiles(durations, n=20)[18] if len(durations) > 20 else max_duration
            )
        else:
            # Valores simulados
        ...
        reliability_score = success_rate * 0.5 + duration_consistency * 0.25 + data_score * 0.25
    ...
    def _generate_performance_recommendations(
        self, metrics: List[ScraperPerformanceMetrics]
    ) -> List[str]:
        ...
        return (
            recommendations
            if recommendations
            else ["✅ Todos os scrapers operando dentro dos parâmetros esperados."]
        )
...
class UptimeReport:
    ...
    def _calculate_component_uptime(
        ...
        sim = uptime_simulations.get(
            component, {"uptime": 99.0, "incidents": 3, "avg_response": 100}
        )
    ...
    def _get_incident_timeline(
        self, metrics: List[UptimeMetrics], start_date: datetime, end_date: datetime
    ) -> List[Dict]:
                    ...
                    incidents.append(
                        {
                            ...
                            "severity": self._get_incident_severity(
                                m.downtime_minutes / m.incidents
                            ),
                        }
                    )
    ...
    def _generate_uptime_recommendations(self, metrics: List[UptimeMetrics]) -> List[str]:
        ...
        return (
            recommendations
            if recommendations
            else ["✅ Todos os componentes operando com uptime satisfatório."]
        )
