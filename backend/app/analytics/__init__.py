...
__version__ = "1.0.0"
__all__ = ["MetricsCalculator", "AnalyticsAggregator"]
...
class MetricsCalculator:
    ...
    def get_overview_metrics(
        ...
        period_investigations = _scalar_count(
            self.db.query(func.count(Investigation.id))
            .filter(
                and_(Investigation.created_at >= start_date, Investigation.created_at <= end_date)
            )
            .scalar()
        )
        ...
        investigations_by_status = (
            self.db.query(Investigation.status, func.count(Investigation.id))
        ...
        # Taxa de conclusão
        completed = status_dict.get(
            InvestigationStatus.COMPLETED.value, status_dict.get("completed", 0)
        )
        completion_rate = (
            (completed / total_investigations * 100) if total_investigations > 0 else 0
        )
    ...
    def get_usage_metrics(
        ...
        completed_investigations = (
            self.db.query(Investigation.created_at, Investigation.updated_at)
            .filter(
                and_(
        ...
        return {
            "daily_activity": [
                {"date": str(date), "count": count} for date, count in daily_investigations
            ],
            "top_users": [
                {
                    "user_id": user_id,
    ...
    def get_scrapers_metrics(
        ...
        properties_count = (
            ...
            .filter(and_(Property.created_at >= start_date, Property.created_at <= end_date))
            .scalar()
        )

        companies_count = (
            self.db.query(func.count(Company.id))
            .filter(and_(Company.created_at >= start_date, Company.created_at <= end_date))
            .scalar()
        )
    ...
    def get_financial_metrics(
        ...
        return {
            "period_metrics": {
                ...
                "average_cost_per_day": round(total_cost / max((end_date - start_date).days, 1), 2),
            },
            "revenue": {
            ...
            "roi": {
                ...
                "margin_percentage": round(((mrr - total_cost) / mrr * 100) if mrr > 0 else 0, 2),
            },
        }
...
class AnalyticsAggregator:
    ...
    def generate_executive_summary(
        ...
        return {
            ...
            "financial": financial,
            "health_score": self._calculate_health_score(overview, performance, financial),
        }
        ...
        return {
            ...
            "data_collected": {
                ...
                "avg_per_investigation": round(
                    (total_properties + total_companies) / total if total > 0 else 0, 2
                ),
            },
        }
        ...
        return {
            "funnel": [
                ...
                {"step": "started", "count": started, "percentage": round(conversion_started, 2)},
                {
                    "step": "completed",
                    "count": completed,
                    "percentage": round(conversion_completed, 2),
                },
            ],
            "drop_off": {
                "created_to_started": round(100 - conversion_started, 2),
                "started_to_completed": round(
                    ((started - completed) / started * 100) if started > 0 else 0, 2
                ),
            },
        }

    def _calculate_kpis(self, overview: Dict, usage: Dict, financial: Dict) -> Dict[str, Any]:
    ...
    def _calculate_health_score(self, overview: Dict, performance: Dict, financial: Dict) -> float:
        """
    ...
    def _generate_operational_recommendations(
        self, overview: Dict, usage: Dict, scrapers: Dict
    ) -> List[str]:
