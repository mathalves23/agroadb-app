...
async def get_user_dashboard(
    ...
    if current_user.id != user_id and current_user.role not in ["admin", "manager"]:
        raise HTTPException(status_code=403, detail="Você só pode acessar seu próprio dashboard")
...
async def get_report_templates(current_user: User = Depends(require_role(["admin", "manager"]))):
    """
...
async def schedule_report(
    ...
    **Requer: admin**
    ...
    result = scheduler.schedule_report(request.config, request.schedule, request.recipients)
    return result
...
async def get_user_analytics(
    ...
    # Validar permissão
    if current_user.id != user_id and current_user.role not in ["admin", "manager"]:
        raise HTTPException(status_code=403, detail="Você só pode acessar suas próprias analytics")
...
async def analytics_health_check():
    ...
    return {
        ...
        "features": {"metrics": True, "dashboards": True, "reports": True, "bi_integrations": True},
    }
