...
class QueueManager:
    ...
    async def enqueue(self, task: Task) -> bool:
        ...
        try:
            ...
            if await self._is_circuit_open(task.type):
                logger.warning(
                    f"⚡ Circuit breaker ABERTO para {task.type.value} - task não enfileirada"
                )
                return False
            ...
            await self.redis_client.setex(
                task_key, timedelta(days=7), json.dumps(task.to_dict())  # TTL de 7 dias
            )
            ...
            await self._update_investigation_progress(
                task.investigation_id, task.id, TaskStatus.PENDING
            )

            logger.info(
                f"✅ Task {task.id} enfileirada: {task.type.value} (prioridade: {task.priority.name})"
            )
            return True
            ...
            for priority in TaskPriority:
                ...
                if result:
                    ...
                    if task_data:
                        ...
                        # Atualizar progresso
                        await self._update_investigation_progress(
                            task.investigation_id, task.id, TaskStatus.RUNNING
                        )
    ...
    async def complete_task(self, task_id: str, result: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Atualizar progresso
            await self._update_investigation_progress(
                task.investigation_id, task.id, TaskStatus.COMPLETED
            )
    ...
    async def fail_task(self, task_id: str, error: str) -> bool:
        ...
        try:
            ...
            if task.retry_count <= task.max_retries:
                ...
                # Atualizar progresso
                await self._update_investigation_progress(
                    task.investigation_id, task.id, TaskStatus.FAILED
                )
    ...
    async def _schedule_retry(self, task: Task, delay: int):
        ...
        await self.redis_client.zadd(retry_key, {task.id: retry_time.timestamp()})
    ...
    async def process_retries(self):
        ...
        try:
            ...
            ready_tasks = await self.redis_client.zrangebyscore(retry_key, min=0, max=now)
    ...
    async def _save_task(self, task: Task):
        """Salva task no Redis"""
        task_key = self._get_task_key(task.id)
        await self.redis_client.setex(task_key, timedelta(days=7), json.dumps(task.to_dict()))

    async def _update_investigation_progress(
        self, investigation_id: str, task_id: str, status: TaskStatus
    ):
        ...
        # Salvar progresso
        await self.redis_client.setex(progress_key, timedelta(days=30), json.dumps(progress))
    ...
    async def _record_failure(self, scraper_type: ScraperType):
        ...
        if failures >= self.CIRCUIT_BREAKER_THRESHOLD:
            ...
            await self.redis_client.setex(
                f"{key}:open", timedelta(seconds=self.CIRCUIT_BREAKER_TIMEOUT), "1"
            )
            try:
    ...
    async def get_queue_stats(self, scraper_type: Optional[ScraperType] = None) -> dict:
        ...
        for stype in scraper_types:
            type_stats = {"total": 0, "by_priority": {}}
