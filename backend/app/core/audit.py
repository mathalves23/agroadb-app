...
class AuditAction(str, Enum):
    ...
    INVESTIGATION_TRUST_EXPORTED = "investigation.trust_exported"
    INVESTIGATION_SHARED = "investigation.shared"
    INVESTIGATION_GUEST_LINK_CREATED = "investigation.guest_link_created"
    INVESTIGATION_GUEST_LINK_REVOKED = "investigation.guest_link_revoked"
    INVESTIGATION_GUEST_ACCESSED = "investigation.guest_accessed"
    INVESTIGATION_GUEST_EXPORTED = "investigation.guest_exported"
    INVESTIGATION_STARTED = "investigation.started"
    INVESTIGATION_CANCELLED = "investigation.cancelled"
    INVESTIGATION_RISK_SCORE_REVIEWED = "investigation.risk_score_reviewed"

    # Machine learning / governança
    ML_RISK_SCORE_COMPUTED = "ml.risk_score_computed"
    ORGANIZATION_AI_GOVERNANCE_UPDATED = "organization.ai_governance_updated"
...
class AuditLogger:
    ...
    @staticmethod
    async def get_user_logs(db, user_id: int, limit: int = 100, offset: int = 0) -> List[AuditLog]:
        """
    ...
    async def get_resource_logs(
        db, resource_type: str, resource_id: str, limit: int = 100
    ) -> List[AuditLog]:
        """
    ...
    async def get_statistics(
        ...
        top_actions = [{"action": row.action, "count": row.count} for row in actions_result]

        # Taxa de sucesso
        success_query = select(AuditLog.success, func.count(AuditLog.id).label("count")).group_by(
            AuditLog.success
        )
        ...
        return {
            ...
            "success_rate": {
                ...
                "percentage": (success_rates.get("success", 0) / total * 100 if total > 0 else 0),
            },
        }
