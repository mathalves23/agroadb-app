...
class LGPDService:
    ...
    @staticmethod
    async def revoke_consent(db, user_id: int, consent_type: ConsentType) -> bool:
        """
    ...
    @staticmethod
    async def check_consent(db, user_id: int, consent_type: ConsentType) -> bool:
        """
    ...
    async def request_data_deletion(
        db, user_id: int, reason: Optional[str] = None
    ) -> DataDeletionRequest:
        """
        ...
        request = DataDeletionRequest(
            user_id=user_id, requested_at=datetime.utcnow(), reason=reason, status="pending"
        )
    ...
    async def process_data_deletion(
        db, request_id: int, processed_by: int, notes: Optional[str] = None
    ) -> bool:
        """
    ...
    @staticmethod
    async def generate_personal_data_report(db, user_id: int) -> Dict[str, Any]:
        """
        ...
        access_stats_query = (
            select(PersonalDataAccess.data_type, func.count(PersonalDataAccess.id).label("count"))
            .where(PersonalDataAccess.user_id == user_id)
        ...
        return {
            ...
            "data_retention_policies": {
                policy.value: days for policy, days in LGPDService.RETENTION_PERIODS.items()
            },
            "rights": {
    ...
    @staticmethod
    async def apply_retention_policy(db, policy: DataRetentionPolicy) -> int:
        """
        ...
        logger.info(
            f"🗑️ Política de retenção aplicada: {policy.value} - {affected} registros afetados"
        )
