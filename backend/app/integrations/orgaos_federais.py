...
class OrgaoFederalIntegration:
    ...
    async def query_ibama(self, cpf_cnpj: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = await self.client.get(f"{self.IBAMA_API}", params={"cpfCnpj": cpf_cnpj})
    ...
    async def query_icmbio(self, coordinates: Dict[str, float]) -> Dict[str, Any]:
        ...
        """
        try:
            response = await self.client.get(f"{self.ICMBIO_API}/unidades", params=coordinates)
    ...
    async def query_cvm(self, cnpj: str) -> Dict[str, Any]:
        ...
        try:
            response = await self.client.get(
                f"{self.CVM_API}/CIA_ABERTA/CAD/DADOS", params={"cnpj": cnpj}
            )
    ...
    async def query_all(
        self, cpf_cnpj: Optional[str] = None, coordinates: Optional[Dict[str, float]] = None
    ) -> Dict[str, Any]:
        """Consulta todos os órgãos"""
        tasks = []

        if cpf_cnpj:
            tasks.extend([self.query_ibama(cpf_cnpj), self.query_cvm(cpf_cnpj)])
