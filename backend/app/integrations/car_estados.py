...
class CARIntegration:
    ...
    async def query_car(self, car_code: str, state: str) -> CARStateResult:
        """
        ...
        try:
            ...
            return CARStateResult(state=state, car_code=car_code, success=False, error=str(e))

    async def query_multiple_states(self, car_codes: Dict[str, str]) -> List[CARStateResult]:
        """
        ...
        """
        tasks = [self.query_car(code, state) for state, code in car_codes.items()]
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                final_results.append(
                    CARStateResult(state=state, car_code=car_code, success=False, error=str(result))
                )
            else:
    ...
    async def _query_via_api(self, car_code: str, state: str) -> CARStateResult:
        ...
        try:
            ...
            return CARStateResult(
                state=state, car_code=car_code, success=True, data=normalized_data
            )

        except httpx.HTTPStatusError as e:
            if e.response.status_code == 404:
                return CARStateResult(
                    state=state, car_code=car_code, success=False, error="CAR não encontrado"
                )
            raise

    async def _query_via_web(self, car_code: str, state: str) -> CARStateResult:
        ...
        try:
            ...
            return CARStateResult(state=state, car_code=car_code, success=True, data=data)

        except Exception as e:
            logger.error(f"Erro no scraping de {state}: {str(e)}")
            raise

    def _normalize_api_response(self, data: Dict[str, Any], state: str) -> Dict[str, Any]:
    ...
    def _parse_car_page(self, soup: BeautifulSoup, state: str) -> Dict[str, Any]:
        ...
        try:
            ...
            owner_elem = soup.find(
                ["span", "div"], text=lambda x: x and "proprietário" in x.lower()
            )
...
async def query_car_all_states(cpf_cnpj: str) -> List[CARStateResult]:
    """
    ...
    car_codes = {state: f"{state}-{cpf_cnpj}" for state in CARStateConfig.URLS.keys()}
...
async def query_car_single(car_code: str, state: str) -> CARStateResult:
    """Consulta CAR único"""
    integration = CARIntegration()
