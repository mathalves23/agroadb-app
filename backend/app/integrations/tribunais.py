...
class TribunalIntegration:
    ...
    async def query_process(
        self, process_number: str, state: str, system: Optional[str] = None
    ) -> ProcessResult:
        """
    ...
    async def query_multiple_processes(
        ...
        """
        tasks = [self.query_process(p["number"], p["state"], p.get("system")) for p in processes]
    ...
    async def _query_esaj(self, process_number: str, state: str) -> ProcessResult:
        ...
        try:
            ...
            return ProcessResult(
                process_number=process_number, state=state, system="esaj", success=True, data=data
            )
    ...
    async def _query_projudi(self, process_number: str, state: str) -> ProcessResult:
        ...
        try:
            # Projudi requer POST
            data = {"numeroProcesso": process_number, "tipoConsulta": "completa"}
    ...
    async def _query_pje(self, process_number: str) -> ProcessResult:
        ...
        try:
            ...
            return ProcessResult(
                process_number=process_number, state="BR", system="pje", success=True, data=data
            )
                ...
                for row in rows:
                    ...
                    if date_elem and desc_elem:
                        movimentacoes.append(
                            {"date": date_elem.text.strip(), "description": desc_elem.text.strip()}
                        )
...
async def query_process_by_number(process_number: str, state: str) -> ProcessResult:
...
async def query_processes_by_cpf_cnpj(
    cpf_cnpj: str, states: Optional[List[str]] = None
) -> List[ProcessResult]:
    """
