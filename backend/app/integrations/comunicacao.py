...
class SlackIntegration:
    ...
    async def _send_via_webhook(
        self, text: str, attachments: Optional[List[Dict]] = None
    ) -> Dict[str, Any]:
        ...
        attachments = [
            {
                ...
                "fields": [
                    {"title": "Score de Risco", "value": f"{risk_score:.1%}", "short": True},
                    {"title": "Padrões Suspeitos", "value": str(patterns_found), "short": True},
                ],
                "footer": "AgroADB",
        ...
        sections = [
            {
                ...
                "facts": [
                    {"name": "Score de Risco:", "value": f"**{risk_score:.1%}**"},
                    {"name": "Padrões Suspeitos:", "value": f"**{patterns_found}**"},
                    {"name": "Status:", "value": "Concluída"},
                ],
            }
        ]
...
class ComunicacaoIntegration:
    ...
    async def notify_all(
        ...
        if self.slack.webhook_url or self.slack.bot_token:
            tasks.append(
                self.slack.send_investigation_alert(
                    investigation_title, risk_score, patterns_found, slack_channel
                )
            )

        if self.teams.webhook_url:
            tasks.append(
                self.teams.send_investigation_alert(
                    investigation_title, risk_score, patterns_found, investigation_url
                )
            )
        ...
        return {
            "slack": (
                ...
                else {"error": str(results[0]) if len(results) > 0 else "Not configured"}
            ),
            "teams": (
                results[1]
                if len(results) > 1 and not isinstance(results[1], Exception)
                else {"error": str(results[1]) if len(results) > 1 else "Not configured"}
            ),
...
async def notify_slack(
    text: str, webhook_url: str, channel: Optional[str] = None
) -> Dict[str, Any]:
...
async def notify_teams(title: str, text: str, webhook_url: str) -> Dict[str, Any]:
...
class ZapierIntegration:
    ...
    async def trigger_zap(self, event_type: str, data: Dict[str, Any]) -> Dict[str, Any]:
        """
...
class DropboxIntegration:
    ...
    async def upload_file(self, file_path: str, dropbox_path: str) -> Dict[str, Any]:
        """
...
class ProductivityIntegration:
    ...
    def __init__(
        ...
        self.gdrive = (
            GoogleDriveIntegration(credentials_json=gdrive_credentials)
                ...
                tasks.append(
                    (
                        ...
                        self.gdrive.upload_file(
                            report_file_path, f"investigation_{investigation_id}_report.pdf"
                        ),
                    )
                )
