...
class OCRProcessor:
    ...
    def process_image(self, image_path: str, language: str = "por") -> OCRResult:
        """
        ...
        if not TESSERACT_AVAILABLE:
            return OCRResult(
                success=False, error="Tesseract não disponível", processing_time_seconds=0.0
            )
        ...
        try:
            ...
            extracted_data = ExtractedData(
                ...
                metadata={"image_path": image_path, "language": language, "image_size": image.size},
            )
            ...
            return OCRResult(
                success=True, extracted_data=extracted_data, processing_time_seconds=processing_time
            )

        except Exception as e:
            logger.error(f"Erro no OCR: {str(e)}")
            processing_time = (datetime.utcnow() - start_time).total_seconds()
            return OCRResult(success=False, error=str(e), processing_time_seconds=processing_time)

    def process_pdf(self, pdf_path: str, language: str = "por", max_pages: int = 10) -> OCRResult:
        """
        ...
        if not PDF2IMAGE_AVAILABLE:
            return OCRResult(
                success=False, error="pdf2image não disponível", processing_time_seconds=0.0
            )
    ...
    def process_base64_image(self, base64_string: str, language: str = "por") -> OCRResult:
        """
    ...
    def _detect_document_type(self, text: str, structured_data: Dict[str, Any]) -> str:
