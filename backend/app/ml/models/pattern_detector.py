...
def _company_label(company: Company) -> str:
    return (company.corporate_name or company.trade_name or company.cnpj or "Empresa").strip()
...
class PatternDetectionResult:
    ...
    def __init__(self, investigation_id: int, patterns: List[Pattern], analyzed_at: datetime):
...
class PatternDetector:
    ...
    def detect(self, investigation_id: int) -> PatternDetectionResult:
        ...
        result = PatternDetectionResult(
            investigation_id=investigation_id, patterns=patterns, analyzed_at=datetime.utcnow()
        )
    ...
    def _detect_land_grabbing(self, investigation: Investigation) -> List[Pattern]:
        ...
        for prop in properties:
            ...
            if area > 1000 and not (has_car or has_matricula):
                confidence += 0.4
                evidence.append(f"Propriedade de {area:.1f} ha sem CAR ou matrícula")
            ...
            if doc_date and area > 500:
                ...
                if doc_date and (datetime.utcnow() - doc_date).days < 1825:  # 5 anos
                    confidence += 0.3
                    evidence.append(f"Documento recente ({doc_date.year}) para propriedade grande")
    ...
    def _detect_money_laundering(self, investigation: Investigation) -> List[Pattern]:
        ...
        if offshore_count > 0:
            ...
            evidence.append(f"{offshore_count} empresa(s) offshore envolvida(s)")
        ...
        total_value = sum(float(_raw(p).get("estimated_value", 0) or 0) for p in properties)

        if total_value > 10_000_000:  # 10 milhões
            confidence += 0.2
            evidence.append(f"Valor total estimado: R$ {total_value:,.2f}")

        # 4. Estrutura societária complexa (múltiplas camadas)
        if len(companies) >= 5:
            confidence += 0.25
            evidence.append(f"Estrutura societária complexa ({len(companies)} empresas)")
    ...
    def _detect_shell_scheme(self, investigation: Investigation) -> List[Pattern]:
        ...
        if duplicate_phones > 0:
            confidence += 0.3
            evidence.append(f"{duplicate_phones} proprietários compartilham telefones")
        ...
        if duplicate_addresses > 0:
            confidence += 0.3
            evidence.append(f"{duplicate_addresses} proprietários compartilham endereços")
        ...
        if common_surnames > 0:
            confidence += 0.2
            evidence.append(f"{common_surnames} sobrenomes compartilhados (possível família)")
        ...
        if len(owners_info) >= len(properties) * 1.5:
            confidence += 0.2
            evidence.append(f"{len(owners_info)} proprietários para {len(properties)} propriedades")
    ...
    def _detect_ghost_properties(self, investigation: Investigation) -> List[Pattern]:
        ...
        for prop in properties:
            ...
            if len(missing_fields) >= 2:
                red_flags.append(
                    f"dados incompletos ({len(missing_fields)} campos essenciais faltando)"
                )
            ...
            if (
                not address
                or len(address) < 10
                or address.lower() in ["n/a", "não informado", "sem endereço"]
            ):
                red_flags.append("endereço ausente ou inválido")

            if len(red_flags) >= 3:
                ghost_properties.append({"name": _prop_label(prop), "red_flags": red_flags})

        if ghost_properties:
            confidence = 0.5 + (len(ghost_properties) * 0.1)
            evidence = [f"{p['name']}: {', '.join(p['red_flags'])}" for p in ghost_properties]
    ...
    def _detect_complex_structures(self, investigation: Investigation) -> List[Pattern]:
        ...
        if layers_count >= 3:
            confidence += 0.4
            evidence.append(f"{layers_count} empresas com sócios PJ (estrutura em camadas)")

        # 3. Empresas offshore
        offshore_count = sum(
            1 for c in companies if "offshore" in (_raw(c)).get("type", "").lower()
        )
    ...
    def _detect_tax_evasion(self, investigation: Investigation) -> List[Pattern]:
        ...
        for company in companies:
            ...
            if "inativa" in status or "baixada" in status or "suspensa" in status:
                ...
                company_properties = [
                    p
                    for p in properties
                    if company.cnpj in str(_raw(p)) or company.cnpj in (p.owner_cpf_cnpj or "")
                ]
        ...
        if inactive_with_properties > 0:
            confidence += 0.5
            evidence.append(f"{inactive_with_properties} empresa(s) inativa(s) com propriedades")
        ...
        if low_value_properties >= 2:
            confidence += 0.3
            evidence.append(f"{low_value_properties} propriedades com valor declarado muito baixo")
        ...
        for company in companies:
            ...
            capital = float(company_data.get("capital_social", 0) or company.capital or 0)
        ...
        if simples_suspeito > 0:
            confidence += 0.4
            evidence.append(f"{simples_suspeito} empresa(s) no Simples com capital social alto")
        ...
        for lease in leases:
            if lease.start_date and lease.end_date:
                if lease.end_date < lease.start_date:
                    confidence += 0.5
                    evidence.append(f"Contrato com data de fim antes da data de início")
        ...
        for lease in leases:
            ...
            if doc_num:
                ...
                if doc_num in ["00000", "11111", "12345", "99999"]:
                    confidence += 0.4
                    evidence.append(f"Número de documento suspeito: {doc_num}")
