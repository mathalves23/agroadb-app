...
class Community:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Converte para dicionário"""
        return {"id": self.community_id, "size": len(self.members), "members": self.members}
...
class NetworkAnalyzer:
    ...
    def _build_graph(self, investigation: Investigation) -> NetworkGraph:
        ...
        for prop in investigation.properties or []:
            ...
            if target_id:
                graph.add_edge(
                    Edge(source=target_id, target=prop_id, relationship_type="owns", weight=1.0)
                )
            ...
            if owner_cpf or owner_cnpj:
                if owner_cpf:
                    ...
                    graph.add_edge(
                        Edge(source=owner_id, target=prop_id, relationship_type="owns", weight=1.0)
                    )
        ...
        for company in investigation.companies or []:
            ...
            if target_id and target_id != company_id:
                graph.add_edge(
                    Edge(
                        source=target_id, target=company_id, relationship_type="related", weight=0.8
                    )
                )
        ...
        for lease in investigation.lease_contracts or []:
            ...
            if lessor_name and lessee_name:
                ...
                if lessor_id not in graph.nodes:
                    graph.add_node(
                        Node(node_id=lessor_id, node_type="person", name=lessor_name, metadata={})
                    )

                if lessee_id not in graph.nodes:
                    graph.add_node(
                        Node(node_id=lessee_id, node_type="person", name=lessee_name, metadata={})
                    )
    ...
    def _calculate_centrality(self, graph: NetworkGraph) -> List[Tuple[str, CentralityMetrics]]:
        """
        ...
        centrality_list.sort(
            key=lambda x: (
                x[1].degree_centrality
                + x[1].betweenness_centrality
                + x[1].closeness_centrality
                + x[1].eigenvector_centrality
            )
            / 4,
            reverse=True,
        )
    ...
    def _bfs_shortest_path(
        self, graph: NetworkGraph, source: str, target: str
    ) -> Optional[List[str]]:
        """BFS para encontrar caminho mais curto"""
        if source == target:
            return [source]
        ...
        for node_id in graph.nodes.keys():
            if node_id not in visited:
                ...
                if len(component) >= 2:  # Pelo menos 2 nós
                    communities.append(Community(community_id=community_id, members=component))
                    community_id += 1
    ...
    def _bfs_component(self, graph: NetworkGraph, start_node: str, visited: Set[str]) -> List[str]:
        """BFS para encontrar componente conectado"""
        component = []
    ...
    def _find_suspicious_paths(
        self, graph: NetworkGraph, investigation: Investigation
    ) -> List[List[str]]:
        """
