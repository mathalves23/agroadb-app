...
def create_optimized_indexes():
    ...
    op.create_index(
        "idx_investigations_created_at", "investigations", ["created_at"], postgresql_using="btree"
    )
    ...
    # Busca por CPF/CNPJ
    op.create_index("idx_investigations_target_cpf_cnpj", "investigations", ["target_cpf_cnpj"])
    ...
    op.create_index(
        "idx_audit_logs_timestamp", "audit_logs", ["timestamp"], postgresql_using="btree"
    )
    ...
    op.create_index("idx_notifications_user_read", "in_app_notifications", ["user_id", "read"])
    ...
    # Busca por categoria
    op.create_index("idx_notifications_category", "in_app_notifications", ["category"])
    ...
    op.create_index("idx_webhook_deliveries_success", "webhook_deliveries", ["success"])

    # ========== TWO FACTOR AUTH ==========
    # Busca por usuário
    op.create_index("idx_two_factor_user_id", "two_factor_auth", ["user_id"], unique=True)

    # ========== LGPD CONSENT ==========
    # Busca por usuário + tipo
    op.create_index("idx_user_consent_user_type", "user_consents", ["user_id", "consent_type"])

    # Busca por ativo
    op.create_index("idx_user_consent_is_active", "user_consents", ["is_active"])

    # ========== DATA DELETION REQUESTS ==========
    # Busca por usuário + status
    op.create_index(
        "idx_deletion_requests_user_status", "data_deletion_requests", ["user_id", "status"]
    )

    # Busca por data de requisição
    op.create_index(
        "idx_deletion_requests_requested_at", "data_deletion_requests", ["requested_at"]
    )
