...
class TestINCRAScraperSearch:
    ...
    async def test_search_by_cpf_cnpj(self, incra_scraper):
        ...
        with patch.object(
            incra_scraper, "_search_by_cpf_cnpj", new_callable=AsyncMock
        ) as mock_search:
    ...
    async def test_search_by_name_fallback(self, incra_scraper):
        ...
        with (
            patch.object(incra_scraper, "_search_by_cpf_cnpj", new_callable=AsyncMock) as mock_cpf,
            ...
            results = await incra_scraper.search(name="João Silva", cpf_cnpj="12345678000190")
...
class TestINCRAScraperCCIR:
    ...
    @pytest.mark.asyncio
    async def test_get_property_by_ccir_api_call(self, incra_scraper, mock_ccir_data):
        ...
        with patch.object(
            incra_scraper, "fetch", new_callable=AsyncMock, return_value=mock_response
        ):
            result = await incra_scraper.get_property_by_ccir(ccir_number)
...
class TestINCRAScraperPropertyCode:
    ...
    @pytest.mark.asyncio
    async def test_get_property_by_code_api_call(self, incra_scraper, mock_property_data):
        """Testa chamada à API quando não está no cache"""
