...
async def test_search_by_matricula(cartorios_scraper):
    ...
    result = await cartorios_scraper.search_by_matricula("12345", state="SP", city="São Paulo")
...
async def test_search_by_owner(cartorios_scraper):
    ...
    results = await cartorios_scraper.search_by_owner(name="João Silva", state="SP")
...
async def test_search_by_owner_cpf(cartorios_scraper):
    ...
    results = await cartorios_scraper.search_by_owner(cpf_cnpj="123.456.789-00", state="SP")
...
async def test_search_by_address(cartorios_scraper):
    ...
    results = await cartorios_scraper.search_by_address("Rua Exemplo, 123", "São Paulo", "SP")
...
async def test_get_certidao(cartorios_scraper):
    ...
    certidao = await cartorios_scraper.get_certidao("12345", "inteiro_teor")
