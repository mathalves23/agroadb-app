...
def test_risk_analyzer_multiple_properties(db: Session, sample_investigation):
    ...
    multiple_props = next((f for f in factor_dict["factors"] if "Propriedade" in f["name"]), None)
...
def test_risk_factor_weighted_score():
    ...
    factor = RiskFactor(
        name="Test Factor", weight=0.5, score=0.8, evidence="Test evidence", severity="high"
    )
...
def test_pattern_detector_ghost_properties(db: Session, sample_investigation):
    ...
    result_dict = result.to_dict()
    ghost_pattern = next(
        (p for p in result_dict["patterns"] if p["pattern_type"] == "ghost_properties"), None
    )
...
def test_node_to_dict():
    ...
    node = Node(
        node_id="test_123", node_type="person", name="Test Person", metadata={"cpf": "12345678900"}
    )
...
def test_risk_analysis_endpoint(
    ...
    gov = data["governance"]
    assert gov.get("engine_version")
    assert gov.get("weights_version")
    assert "indicator_weights" in gov
    assert "methodology_summary_pt" in gov
    assert "human_review_required" in gov


def test_risk_analysis_endpoint_not_found(client: TestClient, auth_headers):
    """Teste com investigação inexistente"""
    response = client.get("/api/v1/investigations/99999/risk-score", headers=auth_headers)
