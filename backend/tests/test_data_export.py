...
class TestRedshiftExporter:
    ...
    def test_execute_copy_command(self, redshift_config):
        """Testa comando COPY"""
        exporter = RedshiftExporter(redshift_config)
        result = exporter.execute_copy_command("test_table", "s3://bucket/path/", "JSON")
...
class TestPowerBIConnector:
    ...
    def test_create_dataset(self):
        ...
        config = PowerBIDataset(
            ...
            tables=[{"name": "investigations", "columns": []}, {"name": "users", "columns": []}],
        )
    ...
    def test_generate_pbix_metadata(self):
        ...
        config = PowerBIDataset(
            dataset_name="Test",
            tables=[{"name": "table1", "columns": [{"name": "col1", "type": "String"}]}],
