...
async def test_search_sigef(sigef_scraper):
    ...
    results = await sigef_scraper.search_sigef(codigo_imovel="SIGEF-001-2024", uf="SP")
...
async def test_search_sigef_by_municipio(sigef_scraper):
    ...
    results = await sigef_scraper.search_sigef(municipio="São Paulo", uf="SP")
...
async def test_search_by_coordinates(sigef_scraper):
    ...
    results = await sigef_scraper.search_by_coordinates(
        latitude=-22.9, longitude=-47.1, radius_km=5.0
    )
...
async def test_convert_datum(sigef_scraper):
    ...
    result = await sigef_scraper.convert_datum(coords, from_datum="SIRGAS2000", to_datum="WGS84")
