...
class TestFunnelAnalytics:
    ...
    def test_get_funnel_comparison(self, mock_db, sample_investigations):
        ...
        comparison = analytics.get_funnel_comparison(
            "investigation_creation", period1_start, period1_end, period2_start, period2_end
        )
...
class TestNPSAnalytics:
    ...
    def test_nps_response_validation(self):
        ...
        response = NPSResponse(
            user_id=1, score=9, category="promoter", created_at=datetime.utcnow()
        )
        ...
        with pytest.raises(ValueError):
            NPSResponse(
                user_id=1, score=11, category="promoter", created_at=datetime.utcnow()  # Inválido!
            )
