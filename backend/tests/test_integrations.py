...
from app.integrations.tribunais import ProcessResult, TribunalIntegration
...
async def test_orgaos_query_all():
    ...
    result = await integration.query_all(
        cpf_cnpj="12345678900", coordinates={"lat": -15.0, "lng": -47.0}
    )
...
def test_car_states_endpoint(client: TestClient, auth_headers):
    ...
    response = client.get("/api/v1/integrations/car/states", headers=auth_headers)
...
def test_tribunal_systems_endpoint(client: TestClient, auth_headers):
    ...
    response = client.get("/api/v1/integrations/tribunais/systems", headers=auth_headers)
...
async def test_full_integration_workflow():
    ...
    # 1. CAR
    car = CARIntegration()
    car_result = CARStateResult(state="SP", car_code="SP-123", success=True, data={"test": "data"})
